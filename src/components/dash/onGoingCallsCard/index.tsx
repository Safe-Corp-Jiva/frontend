    ...
    <div className="w-full h-full bg-white rounded-xl flex flex-col p-4">
      ...
      <div className="h-full overflow-y-auto flex flex-col">
        {OnGoingCalls.length === 0 ? (
          <h1 className="text-center text-gray-400 my-auto">No ongoing calls</h1>
        ) : (
          OnGoingCalls.map((call, index) => (
            <button
              onClick={handleCallClick(call.id)}
              className={`flex flex-row justify-between space-x-8 text-center items-center border-b-2 border-gray-200 ${
