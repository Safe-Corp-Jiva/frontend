...
import { topicSubFactory } from '@/utils/gql'
...
export default function TrendingCard({ maximize, minimize, isMaximized }: Props) {
  ...
  useEffect(() => {
    const { getter, updateSub } = topicSubFactory()
    getter().then((topics) => {
      setTrendingTopics(topics.sort((a: any, b: any) => b.count - a.count));
      setLoading(false);
    });
      ...
      <div className="h-full overflow-y-auto">
        {
          loading ? (
            <div className='flex flex-col justify-center items-center'>
              <svg aria-hidden="true" className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-SCJ-primary" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          trendingTopics.map((topic: any, index: number) => (
            ...
            >
              ...
              <h1 className="flex-1 font-semibold">{topic.name}</h1>
              <h1 className="text-right text-gray-400">{topic.count}</h1>
