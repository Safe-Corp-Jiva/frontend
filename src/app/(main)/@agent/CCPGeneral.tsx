...
const ConnectCCP = () => {
  ...
  return incomingCall && !callAccepted ? (
    <div
      className={`absolute top-4 right-1/2 translate-x-1/2 flex flex-row gap-4 items-center justify-center bg-white rounded-xl p-2 shadow-md ${
    ...
    callAccepted && (
      <div className="sticky right-0 top-0 h-full flex items-center justify-center bg-SCJ-gray">
        <div
          className={`flex flex-col items-center justify-center bg-white rounded-xl mr-4 p-2 gap-4 shadow-md ${
            pathname == '/dashboard' && 'hidden'
          }`}
        >
          {/* <p>Call in progress</p> */}
          <p>{startTimestamp && currentTimestamp && formatTimestampDifference(startTimestamp, currentTimestamp)}</p>
          <div>
            <button 
